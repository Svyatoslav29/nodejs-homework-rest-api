...
const listContacts = async (userId, { sortBy, sortByDesc, filter, limit = 10, skip = 0 }) => {
  ...
  let result = Contact.find({owner: userId}).populate({path: 'owner', select: 'name email role'});
  if (sortBy) {
...
const getContactById = async(userId, contactId) => {
  const result = await Contact.findOne({_id:contactId, owner: userId}).populate({path: 'owner', select: 'name email role'});
  return result;
}
